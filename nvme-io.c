...
//#ifdef CONFIG_SSD_READ
/******check cdw12,25bit***********/
    if (cmd->cdw12 & (1 << 25)) {
        switch(slba) {
        case 1:
            // receive data for searching
            void *LS = n->mbe->logical_space;
            char *filepath = g_malloc0(4096 * sizeof(char));
            n->mbe->logical_space = filepath;
            data_offset = 0;
            ret = backend_rw(n->mbe, &req->qsg, &data_offset, 1);
            n->mbe->logical_space = LS;
            printf("nvme_rw: received buffer:\r\n\tfilepath: %s\r\n", filepath);

            // do search
            req->cqe.n.result = nvme_search(n->mbe, filepath); // search func.
            req->cqe.n.rsvd = (1 << 1);
            printf("nvme_rw: searched destination inode #: %d\r\n", req->cqe.n.result);

            if (2 <= req->cqe.n.result) {
                req->cqe.n.rsvd |= (1 << 0);
            }
            break;
        default:
            printf("nvme_rw: Wrong command!\r\n");
            break;
        }
        return NVME_SUCCESS;
    }
...
//#endif
    ...
    if (!ret) {
        FILE *fp = fopen("/tmp/ramdisk/read_count.csv", "a+");
        fprintf(fp, "%ld, %d, %ld\n", req->slba / 8, req->nlb / 8, qemu_clock_get_ns(QEMU_CLOCK_REALTIME));
        fclose(fp);
        return NVME_SUCCESS;
    }
